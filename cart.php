...
<body id="page-top">
  ...
  <div  class="intro route bg-image" style="background-image: url(img/mavin.jpg)">
    ...
    <div class="intro-content display-table">
      <div class="table-cell">
        <div class="container">
          <h1 class="intro-title mb-4">CART</h1>
          ...
          <h1 class="pt-3"><a class="btn btn-default btn js-scroll px-4" style="background-color:#be613d;color:antiquewhite;" href="#about" role="button">
              <?php
 ...
 <br>
  <?php
    if(isset($_SESSION['succ_order'])){
      echo  '<div class="alert alert-success">
      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
      <strong>'. $_SESSION['succ_order'].'!!!</strong>
    </div>';
     }elseif(isset($_SESSION['error_order'])){
      echo  '<div class="alert alert-success">
      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
      <strong>'. $_SESSION['error_order'].'!!!</strong>
    </div>';
     }else{
  ...
  <div class="container">
     <?php
     if(isset($_SESSION['succ_cart'])){
      echo  '<div class="alert alert-success">
      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
      <strong>'. $_SESSION['succ_cart'].'!!!</strong>
    </div>';
     }elseif(isset($_SESSION['err_cart'])){
      echo  '<div class="alert alert-success">
      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
      <strong>'. $_SESSION['err_cart'].'!!!</strong>
    </div>';
     }else{

     }
    $rerun="SELECT * from reg_user where pass=".$_SESSION["pass"];
   if($q_run =mysqli_query($conn, $rerun)){
     
   }else{
     echo mysqli_error($conn);
   }
    $row_run = mysqli_fetch_assoc($q_run);
    $reg= $row_run['reg_id'];

echo "<div class='row'>";
     $sql = "SELECT * FROM cart_table WHERE reg_id='$reg'";
     $result = mysqli_query($conn, $sql);
     if (mysqli_num_rows($result) > 0) {
       // output data of each row
       while($row = mysqli_fetch_assoc($result)) {
         require ('c_prod_call.php');
       }
   } else {
       echo "No products in cart Yet!!!";
   }
   echo"</div>";
     ?>
      </div>
