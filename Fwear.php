...
<body id="page-top">
 ...
 <div class="conatiner">
 <br>
  <h3 class="intro-title mb-4" style= "text-align:center;">FOOT WEARS</h3>
  </div>
  ...
  <div class="container">
        ...
        <ul class="nav nav-tabs nav-justified" role="tablist">
          <li class="nav-item">
            <a class="nav-link active" data-toggle="tab" href="#laptop">slippers/sandals</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" data-toggle="tab" href="#phone">Brooks</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" data-toggle="tab" href="#speaker"> Cooperate shoes</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" data-toggle="tab" href="#tv">random collection</a>
          </li>
        </ul>
        ...
        <div class="tab-content">
         <div id="laptop" class="container tab-pane active"><br>
           ...
           <!--tab for Cooperate shoes-->
           
              <div id="speaker" class="container tab-pane fade"><br>
              <div class="row">
              <?php
              $sql = "SELECT id, pname, price, image_dir, tel, category, descr FROM products WHERE category= 'Cooperate shoes' ";
              $result = mysqli_query($conn, $sql);
              if (mysqli_num_rows($result) > 0) {
                // output data of each row
                while($row = mysqli_fetch_assoc($result)) {
           ...
           <!--tab for random collection-->
         
              <div id="tv" class="container tab-pane fade"><br>
              <div class="row">
              <?php
              $sql = "SELECT id, pname, price, image_dir, tel, category, descr FROM products WHERE category= 'random collection'";
              $result = mysqli_query($conn, $sql);
              if (mysqli_num_rows($result) > 0) {
                // output data of each row
                while($row = mysqli_fetch_assoc($result)) {
          ...
          <div class="row">
              <?php
              $sql = "SELECT id, pname, price, image_dir, tel, category, descr FROM products WHERE category= 'Brooks' ";
              $result = mysqli_query($conn, $sql);
              if (mysqli_num_rows($result) > 0) {
                // output data of each row
                while($row = mysqli_fetch_assoc($result)) {
